...
**⚠️ 重要提示：本技能的静态检查步骤依赖本机 DevEco 工具链**

原始版本依赖 DevEco Studio CodeGenie 的 MCP 工具，本环境不存在。按下表替换：

| 原 MCP 工具 | DSH 替代方式 |
| --- | --- |
| `mcp_codegenie-mcp_check_ets_files` | 无 1:1 等价物。优先用 `pwsh` 运行 `hvigorw`，让 ArkTS 编译器报出 code 6387 / deprecatedSymbol 诊断；快速静态检查可加载 `hmos-arkts-knowledge-retriever` skill，使用其中的 `linter-cli`。两者都不可用时退化为人工审查，并在报告中注明。 |
| `mcp_codegenie-mcp_build_project` | `pwsh` 执行工程根目录的 `hvigorw` |
| `mcp_codegenie-mcp_harmonyos_knowledge_search` | 加载 `hmos-arkts-knowledge-retriever` skill，或使用 `web_search` |

前提：已安装 DevEco Studio 并配置 `DEVECO_SDK_HOME`，工程根目录存在 `hvigorw`（Windows 为 `hvigorw.bat`）。
...
2. **调用静态检查工具**
   ...
   ```powershell
   # 让 ArkTS 编译器报出废弃接口诊断（得到 code 6387 / deprecatedSymbol）
...
| 工具 | 用途 |
| --- | --- |
| `pwsh` + `hvigorw` | 构建工程并获取 ArkTS 编译器诊断（含 6387 废弃接口） |
| `glob` | 搜索项目中的 ETS 文件 |
| `grep` | 按名称搜索废弃/替代 API |
| `read` / `edit` | 读取与修复源码 |
| `web_search` / `web_fetch` | 查询 HarmonyOS 官方文档 |
### 常见错误码
