...
description: "Push Token 获取助手，可作为单独接入能力使用。当开发者需要集成华为推送服务、首次获取 Push Token、或 Token 获取失败报错（如 1000900010）时触发，也适用于需要在项目中添加 getToken 代码的场景。触发示例：\"帮我接入 push token\"\"获取 push token\"\"首次集成推送功能\"\"getToken 调用失败\"。hmos-push-kit-notification、hmos-push-kit-voip、hmos-push-kit-background 在接入前都会先检查 Token 状态，未接入时会引导开发者先使用本 Skill。"
---
