...
description: "ArkUI 代码开发助手，面向 HarmonyOS UI 开发，提供基于知识库的 UI 开发能力，内部调用 hmos-arkui-knowledge-retriever 获取 API 证据，包含编码约束规则和最佳实践参考。触发场景：(1) 要求生成 ArkUI 页面或组件；(2) 在现有 .ets 工程上要求增删改功能；(3) 提供报错/截图要求修复 ArkUI 代码；(4) 提到 HarmonyOS/ArkUI/ArkTS/.ets；(5) 提到状态管理、组件、布局等界面相关开发任务并期望得到可运行代码。涉及架构决策（ViewModel 提取、状态归属判断、目录规划、MVVM 整改）时应先加载 hmos-arkui-mvvm-pattern。"
---
...
1. 先读 `references/quick-apis/_index.md`：在「组件索引」查组件所在文件，再用 `grep` 按 `^### 组件名` 在该文件中定位卡片起始行，然后用 `read` 带 `offset`/`limit` 读取后续 30–50 行（**勿整读分类文件**，01-layout/02-basic 含数十卡片，整读浪费 10×+ token）
