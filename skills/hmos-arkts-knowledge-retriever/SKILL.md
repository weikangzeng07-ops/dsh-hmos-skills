...
description: "Retrieve grounded ArkTS references for non-UI ArkTS work and ArkTS API usage. Use whenever the user writes, reviews, tests, or debugs ArkTS code and the answer must come from the bundled sources (`docs/ArkTS-Language-Guide/`, `docs/ArkTS-API-Reference/`, `docs/linter/`, `linter-cli/`) rather than model memory. Covers syntax rules, ArkTS restrictions, API/module/member lookup, common-library usage, lightweight lint workflow, runnable examples, and which source to trust before suggesting fixes."
---
