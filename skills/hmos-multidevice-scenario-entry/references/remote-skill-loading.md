...
```bash
./scripts/remote_load.sh <skill-name> dsh -d .dsh/skills
```

示例 — 加载单个场景 skill：

```bash
./scripts/remote_load.sh hmos-multidevice-screen-window-size dsh -d .dsh/skills
```

示例 — 加载多个场景 skill（需逐个调用）：

```bash
./scripts/remote_load.sh hmos-multidevice-fold-state dsh -d .dsh/skills
./scripts/remote_load.sh hmos-multidevice-interaction-methods dsh -d .dsh/skills
```
...
入口 skill 位于 `<workspace>/.dsh/skills/hmos-multidevice-scenario-entry/`，场景 skill 安装到同级：

```text
<workspace>/.dsh/skills/hmos-multidevice-screen-window-size/
<workspace>/.dsh/skills/hmos-multidevice-interaction-methods/
```
