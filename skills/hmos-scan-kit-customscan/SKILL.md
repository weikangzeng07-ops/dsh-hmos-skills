...
**注意**：
- 本技能已移植到 DeepSeek Harness。原文中依赖 CodeGenie 内置工具的「语法检查 / 编译校验」步骤，请改用 `pwsh` 调用工程根目录的 `hvigorw`（需已安装 DevEco Studio 并配置 `DEVECO_SDK_HOME`）；工具链不可用时退化为静态审查并明确说明未实际编译。
...
在开始工作流程前，**必须** 先使用 `todo_write` 工具创建包含5项任务的待办事项，在完成每一个阶段之后标记为完成
