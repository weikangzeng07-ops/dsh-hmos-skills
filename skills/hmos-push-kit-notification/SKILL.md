...
description: "发送通知消息助手。当开发者需要实现推送通知功能、发送消息提醒、配置通知样式或点击动作时触发。触发示例：\"帮我在项目中接入推送通知\"\"实现推送消息功能\"\"添加推送消息功能\"\"接入通知消息\"\"发送通知消息\"\"配置推送前台接收\"。前置检查：继续之前会自动检查开发者是否已接入 Push Token，未接入则先引导使用 hmos-push-kit-token。本 Skill 专注于通知消息的发送和配置，不适用于仅询问概念、测试调试或问题排查。"
---
