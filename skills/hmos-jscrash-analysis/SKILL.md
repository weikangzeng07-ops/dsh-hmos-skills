...
description: "DFX 技能：分析 HarmonyOS/OpenHarmony 应用的 JS Crash（ArkTS/JS 层闪退）faultlogger 日志，按 Reason、Error name、Error message、Error code 与 Stacktrace 定位根因并给出修复建议。当用户提供含 JS Crash、Reason:Error/TypeError/SyntaxError/ReferenceError/RangeError/BusinessError/OutOfMemoryError、Error message、Stacktrace、HybridStack、faultlogger、Cannot get SourceMap info 等字段的日志，或询问应用启动/点击后闪退、ArkTS 崩溃、JS Crash 定位、OOM 闪退原因时必须使用。确认 OOM 且同时提供 rawheap/heapsnapshot 时继续调用 hmos-jsleak-analysis。日志为 cppcrash、freeze 或无 JS/ArkTS 错误字段的原生崩溃时不要使用。"
metadata:
